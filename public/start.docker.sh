...
{ # this ensures the entire script is downloaded
  ...
  nosana_run_cmd() {
    docker ps -a -q --filter "ancestor=docker.io/nosana/nosana-node:latest" | xargs -r docker rm -f
    docker images -f "dangling=true" -f "reference=docker.io/nosana/nosana-node:latest" -q | xargs -r docker rmi -f

    if ! docker network ls | grep NOSANA_GATEWAY > /dev/null 2>&1; then
      docker network create --driver bridge --subnet=192.168.101.0/24 --gateway=192.168.101.1 NOSANA_GATEWAY > /dev/null 2>&1
    fi
    ...
    NOSANA_NODE_ARGS+=(--docker /var/run/docker.sock)
